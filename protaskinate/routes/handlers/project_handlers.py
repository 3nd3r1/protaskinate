...
from protaskinate.routes.forms.project_forms import (CreateCommentForm, CreateProjectForm,
...
def handle_create_project(form: CreateProjectForm):
    ...
    project_service.create(name=data["name"], creator_id=current_user.id,
...
def handle_create_task(project_id: int, form: CreateTaskForm):
    ...
    data = {
        ...
        "assignee_id": form.assignee_id.data \
            if form.assignee_id.data != 0 else None,
    ...
    task_service.create(title=data["title"],
                        status=data["status"],
