...
from protaskinate.routes.forms.project_forms import (CreateCommentForm,
                                                     CreateProjectForm,
                                                     CreateProjectUserForm,
                                                     CreateTaskForm)
from protaskinate.routes.handlers.project_handlers import (
    handle_create_comment, handle_create_project, handle_create_project_user,
    handle_create_task, handle_update_task, handle_update_user_role)
...
def project_view_route(project_id: int):
    ...
    create_project_user_form = CreateProjectUserForm(request.form, prefix="create_project_user")

    if create_task_form.submit.data and create_task_form.validate_on_submit():
        handle_create_task(project_id, create_task_form)

    if create_project_user_form.submit.data and create_project_user_form.validate_on_submit():
...
@blueprint.route("/projects/<int:project_id>/users/<int:user_id>/edit", methods=["POST"])
@login_required
@project_update_access_required
